...
export default {
  ...
  methods: {
    async prepareChartData() {
      let that = this;
       await axios.get('https://makaaziserverapi-production-252f.up.railway.app/api/charts/trends/'+that.estateId,{})
                .then(function (response) {
                    if (response.status == 200) {
                       ...
                       console.log(that.data)
                        that.chartData.labels = that.data.map((data) => data.month);
      that.chartData.datasets[0].data = that.data.map(
        (data) => data.total_registrations
      );
